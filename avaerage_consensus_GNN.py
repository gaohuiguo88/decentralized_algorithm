...
parser.add_argument('--num_weight', type=int, default=3,
                    help='number of weight of a node')
...
os.makedirs('./try_%s_Dec7th'%args.obj, exist_ok=True)
os.chdir('./try_%s_Dec7th'%args.obj)
...
for i in range(args.data_size):
    ...
    if args.distribution == 'normal':
        ...
        x = torch.Tensor(np.random.binomial(50,0.5,[args.num_node,args.num_weight]))
    elif args.distribution == 'exp':
        x = torch.Tensor(np.random.exponential(scale=1.0, size=[args.num_node,args.num_weight]))
    elif args.distribution == 'lognormal':
        x = torch.Tensor(np.random.lognormal(mean=0.0, sigma=1.5, size=[args.num_node, args.num_weight]))
    else:
    ...
    if args.obj == 'average':
        ...
        y = (np.median(x,axis=0).item() * torch.ones(args.num_node))
    elif args.obj == 'max':
        y = (max(x.squeeze()) * torch.ones(args.num_node))
        # print(y)
    else:
...
model = GCN(nfeat=args.num_weight, nhid=32, K=args.K+1, nclasses=args.num_weight)
